...
db.serialize(function() {
  ...
  //   FROM Status JOIN Players
  //   ON Players.id = Status.playerId
	// ORDER BY level DESC`, (err,data) => {
  //     console.log(data);
  //   })

  //Release 1
  db.all(`SELECT COUNT(*) AS totalcard, Cards.playerId, Players.name, Players.age, Players.gender
    FROM Cards LEFT JOIN Players
    ON Cards.playerId = Players.id
    WHERE Players.gender = 'Male'
    GROUP BY Players.name
  	ORDER BY totalcard DESC, Cards.playerId ASC
  	LIMIT 3`,(err,data) => {
        console.log(data);
    })
});
